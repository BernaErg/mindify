...
(function () {
  ...
  function render() {
    ...
    $("bar").style.width = pct + "%";
    $("p-count").textContent = (t.progress || "{d}/{t}")
      .replace("{d}", g.done).replace("{t}", g.total).replace("{n}", ALL.length);
    $("p-pct").textContent = pct + "%";
    $("u-sub").textContent = ALL.length === 1 ? ALL[0].title
      : (t.coursesInProgress || "{n}").replace("{n}", ALL.length);
    ...
    $("courses").innerHTML = ALL.map(function (c) {
      ...
      return '<div class="course-block" data-slug="' + c.slug + '">' +
        ...
        '<p class="muted" style="margin:0;font-size:.92rem">' + loc(c, "subtitle") + '</p></div>' +
        '<span class="pill' + (complete ? ' pill-done' : '') + '">' + list.length + ' / ' + c.modules.length + ' ' + t.complete + '</span></div>' +
        ...
        c.modules.map(function (m) {
          ...
          return '<div class="mod-row ' + (isDone ? "done" : "") + '">' +
            ...
            (isDone ? '<span class="pill pill-done">' + t.completeTag + '</span>' : '') +
            '<a class="btn btn-ghost btn-sm" href="module.html?c=' + c.slug + '&m=' + m.n + '">' + t.open + '</a></div>';
        }).join("") +
        '</div><aside>' +
          '<div class="cert-panel' + (complete ? ' unlocked' : '') + '">' +
            '<div style="width:46px;height:46px;margin:0 auto 14px;color:var(--blue-500)">' + CERT + '</div>' +
            '<h3 style="font-size:1.1rem">' + (complete ? t.certReady : t.certTitle) + '</h3>' +
            '<p class="muted" style="font-size:.9rem">' + (complete
              ? (t.certDone || "").replace("{n}", c.modules.length)
              : (t.certToGo || "").replace("{n}", c.modules.length - list.length)) + '</p>' +
            '<button class="btn btn-primary btn-block cert-btn" data-slug="' + c.slug + '" type="button"' +
            (complete ? "" : " disabled") + ' style="margin-top:14px">' + t.certBtn + '</button>' +
          '</div>' +
          '<div class="card" style="margin-top:14px"><h4>' + t.nextH + '</h4>' +
  ...
  function toggle(slug, n) {
    ...
    A.setModuleComplete(slug, n, willBeDone).catch(function (err) {
      ...
      $("dash-err").textContent = t.errSave + " " + (err.message || "?");
  ...
  A.requireUser().then(function (u) {
    ...
    $("dash-err").textContent = t.errLoad + " " + ((err && err.message) || "?") + ". " + t.errRetry;
    $("dash-err").classList.add("show");
  });
})();
